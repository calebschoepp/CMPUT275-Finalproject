...
ImprovedHumanHeuristic::ImprovedHumanHeuristic(
        queue<gridNum> *displayQueue, gridArr boardCopy) {
...
void ImprovedHumanHeuristic::removeAll(posInfo (&probBoard)[9][9],
        gridNum &curSpot, bool output) {
...
void ImprovedHumanHeuristic::setboard(gridArr board,
            posInfo (&probBoard)[9][9]) {
...
void ImprovedHumanHeuristic::checkPos(posInfo (&probBoard)[9][9], int row,
        int col, bool output) {
    /*
...
void ImprovedHumanHeuristic::removeRow(posInfo (&probBoard)[9][9],
        gridNum &curSpot, bool output) {
...
void ImprovedHumanHeuristic::removeCol(posInfo (&probBoard)[9][9],
        gridNum &curSpot, bool output) {
...
void ImprovedHumanHeuristic::removeSquare(posInfo (&probBoard)[9][9],
        gridNum& curSpot, bool output) {
...
gridArr ImprovedHumanHeuristic:: setNewBoard(posInfo (&probBoard)[9][9]) {
    // translates the current probability board into a gridArr type.
    gridArr newBoard;
    for (int row = 0; row < 9; row++) {
        for (int col = 0; col < 9; col++) {
            newBoard[row][col] = probBoard[row][col].num;
        }
    }
    return newBoard;
}

bool ImprovedHumanHeuristic::solveHeuristically(gridArr board, bool output) {
    // solves the sudoku board using the human heuristic algorithm. If the
    // board can be completely filled it returns true, otherwise it uses the
    // backtracking algorithm with the new known points to get the rest of the
    // positions faster than backtracking by itself.
    ...
    if (totalKnown == 81) {
        ...
        gridArr newBoard = setNewBoard(probBoard);
